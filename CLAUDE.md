...
- **2026-05-04** Convención técnica — NO usar el bloque `env` en `next.config.js` para variables que pueden ser `undefined` en build time. Ese bloque fuerza a Next.js a inlinear el valor en build en TODO el bundle (incluso server code), pisando el comportamiento normal de runtime de variables `NEXT_PUBLIC_*`. Si la env no está disponible cuando corre `npm run build` (caso típico: Dockerfile que no la setea como `ARG`/`ENV`), Next.js inlinea el fallback del `||` y queda hardcoded forever en el JS compilado, ignorando lo que después setees en runtime. Para variables server-only que dependen del entorno (URLs públicas), no usar prefijo `NEXT_PUBLIC_` y leer `process.env.X` directamente runtime. _(Origen: BUG-010 — preferences MP creadas con `back_urls=""` y `notification_url=http://localhost:3000` rompiendo todas las compras reales con CPT01)_
...
- **2026-05-06** Convención técnica — **Drizzle + SQLite/Turso: NUNCA confiar en `default(sql\`CURRENT_TIMESTAMP\`)` para columnas declaradas como `integer({ mode: 'timestamp' })`**. `CURRENT_TIMESTAMP` en SQLite devuelve string ISO `"YYYY-MM-DD HH:MM:SS"`, no integer unix epoch. Drizzle solo convierte `Date → integer` cuando vos pasás un `new Date()` explícito en el INSERT — los defaults SQL pasan de largo. Resultado: la columna queda como TEXT, y queries con filtro temporal (`lte(table.createdAt, someDate)`) NUNCA matchean porque comparan string vs integer. **Regla**: en cada `db.insert` o `tx.insert` a una tabla con `mode:'timestamp'`, pasar `createdAt: new Date(), updatedAt: new Date()` explícito. Sin excepción. _(Origen: BUG-012 — el cron `releaseExpiredOrders` jamás cancelaba orders pending viejos en producción porque `orders.created_at` se guardaba como string; descubierto al inspeccionar un order real esperando 15min de timeout)_

- **2026-05-06** Convención técnica — **Cuando una columna FK puede recibir IDs de tablas distintas según contexto (caso `event_logs.purchase_id` con FK a `purchases.id` mientras el dominio también maneja `combo_purchases.id` con prefijo `COM-`), nunca insertar el ID de la otra tabla en esa columna**. Con `PRAGMA foreign_keys=1` (default en Turso) el INSERT falla con SQLITE_CONSTRAINT. Patrón seguro: pasar la FK como `null` + el ID semántico (`comboChildId`, `legacyRef`) en el campo `data` JSON. Auditoría preservada por `order_id` u otra FK válida. _(Origen: BUG-013 — capturado en gate concurrency tests pre-Fase 7.D, 3 inserts buggy en orderService + webhook. Sin el gate de 3 runs habría llegado a producción)_

- **2026-05-06** Convención de proceso — **Auditoría con 4 agents paralelos** (payment-flow-debugger + concurrency-validator + general-purpose code-reviewer + db-migration-reviewer) en una sola tool call funciona muy bien para cobertura amplia post-deploy. Lanzar especialistas distintos sin solapamiento ahorra tiempo y reduce ruido vs un único agente generalista. Cada uno con prompt self-contained que delimita explícitamente "no repitas X que cubre el otro". 4 agents en ~25min wall-clock. Resultado en sesión 8: 0 critical, 7 important detectados (3 fixeados pre-cierre), 8+ minor. _(Origen: auditoría post-deploy Fase 7.D, descubrió 2 issues important que escaparon a los reviewers in-line)_
