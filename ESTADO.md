...
- **Estado actual**: **Sistema RESETEADO y reabierto para la SEGUNDA SEDE del colegio** (2026-06-09). Nueva rifa **id=3 "Rifa Escolar 2026 - Sede 2"**, 2000 números disponibles, **$1.000 c/u**, misma cuenta MercadoPago, revision Cloud Run **`00021-fdd`**, **ventas ABIERTAS**. Backup completo de sede 1 (145 orders approved, $4.798.000, 4.568 filas, 8 tablas) en `backups/rifa-sede1-final-backup-2026-06-09.json` (gitignored). Gate de fecha hardcoded de sede 1 REMOVIDO; cierre ahora vía soft gate `is_active` (hasta que se implemente 12.1). Bingo Escolar sigue operativo sin cambios (https://bingo-escolar-kc5dasqukq-ue.a.run.app, client-side puro, la sede 2 lo reusa tal cual). **Fase 12 EN PROGRESO**: 12.1 (cierre auto 6/7 23:59) + 12.2 (combo empanadas **$12.000** con reparto de gustos carne/jamón y queso) **DEPLOYADAS a producción** (revision `00022-hzf`, smoke prod verde) — app lista para difundir. 12.3 (tickets alumno-primero + desglose de gustos) implementada local-only. Falta solo 12.4 (talonario de no vendidas — espera medidas exactas + lema del colegio).
- **Última sesión**: 2026-06-12 — Save #14 — Fase 12.1 + 12.2 + 12.3 (combo empanadas $12.000 con gustos + cierre 6/7 + tickets alumno-primero). 12.1/12.2 deployadas a prod (`00022-hzf`, luego `00023-zxz` con fecha 6/7); 12.3 local-only. Schema `flavor_breakdown` aplicado, reviews db-migration + payment-flow APROBADAS, smoke prod E2E verde (BUG-010 OK). Falta 12.4 (espera medidas+lema) + compra real de tercero. Backup de las 8 tablas (sucesor del script viejo que sólo cubría 5) + reset destructivo atómico revisado por `db-migration-reviewer` (APROBADO) + rifa nueva id=3 + redeploy `00021-fdd` + smoke prod verde (2000/2000 disponibles). Removido el gate `SALES_CLOSE_TS` de sede 1. Plan completo de los 5 pedidos de los directores de sede 2 en `docs/superpowers/plans/2026-06-09-fase-12-cambios-sede2.md` (el #5 se fusionó en el combo). Decisiones del combo cerradas: único combo empanadas, suma de gustos exacta N×2.
...
- [x] 12.1 Cierre automático lunes 6/7 23:59 ART — **igual que sede 1** (gate de UI hardcodeado): reintroducido en `RifasApp.tsx` `SALES_CLOSE_TS = new Date('2026-07-06T23:59:00-03:00')` + `useState` lazy init + `useEffect` con `setTimeout(remaining)` para cruzar la medianoche sin refresh + hard gate `if (salesClosed) return <SalesClosedScreen/>` (prioritario, antes del isLoading, aparece aunque la API esté caída). Soft gate `is_active` mantenido. **Deployado 2026-06-12 (revision `00022-hzf`), smoke prod verde.** - DEV
- [x] 12.2 Combo empanadas con selección de gustos + cantidad (fusiona ex-12.5) — **único combo** = "Combo de empanadas" (2 empanadas + gaseosa, **$12.000** confirmado por directores 2026-06-12; sandwiches carne/chorizo eliminados). Elegir N combos → repartir **exactamente N×2 empanadas** entre Carne / Jamón y queso (steppers con suma exacta, botón continuar bloqueado hasta cuadrar). **Schema:** columna additive nullable `flavor_breakdown TEXT` (JSON `{"carne":x,"jyq":y}`) en `combo_purchase_items` — `quantity` sigue = nº combos, `unitPrice` = $12.000 (NO se tocó la semántica de precio → flujo de pago intacto). `db-migration-reviewer` APROBADO, ALTER aplicado a Turso prod (BD sede 2 vacía). Tocó: `lib/combos.ts`, `components/combos/ComboCatalog.tsx` (reescrito como picker), `ComboRow.tsx` (borrado), `OrderFlow`, `CartDrawer`, `UnifiedReview`, `OrderSuccessScreen`, `ProductSplitHero`, `CrossSellSheet`, `orderService.createOrder` (valida N×2 + persiste desglose), Zod purchase route (+400 claro), preference route (título MP con desglose), `scripts/generar-supermercado-csv.mjs` (totales por gusto). `payment-flow-debugger` APROBADO. Validado E2E local (creación + persistencia + preference + rollback) contra BD prod, registros de prueba limpiados. **Deployado 2026-06-12 (revision `00022-hzf`), smoke prod verde.** - DEV
- [x] 12.3 Impresión de tickets: **alumno primero** (+curso, destacado EB Garamond), al lado el **adulto** comprador ("compró: ..."); combos de empanadas con **desglose de gustos** (Carne ×N / J. y queso ×N) + números rifa. Campo alumno único (sin separar). Hecho: `lib/tickets/queries.ts` (orderBy → `studentName`; trae `flavorBreakdown`), `lib/tickets/render.ts` (bloque invertido + chips por gusto), `lib/tickets/styles.ts` (`.p-alumno`/`.p-curso`/`.p-adulto`), `app/admin/tickets/page.tsx` (tabla reordenada Alumno→Curso→Adulto). Sin schema. Validado visualmente con render mock (4 casos: rifa+combo, combo-only con gustos, solo-combos sin alumno, solo-rifa). Local-only (404 en prod). **Implementado 2026-06-12.** - DEV
- [ ] 12.4 Talonario de rifas NO vendidas (venta en puerta): script nuevo `scripts/generar-talonario-novendidas.mjs` (query `status='available'`), formato talonario clásico (número impreso 2× con línea de corte punteada) + branding colegio (nombre/escudo/lema). **Medidas exactas PENDIENTES** (las pasa el usuario). Ejecutar POST-cierre 6/7 (evitar doble venta puerta-vs-online). - DEV

---

## Bitácora

### 2026-06-12 — Save #14 (Fase 12.1 + 12.2 + 12.3: combo empanadas con gustos, cierre 6/7, tickets alumno-primero)
- **Tareas completadas**: **12.1** (cierre auto 6/7 23:59 ART, en prod) + **12.2** (combo empanadas $12.000 con reparto exacto de gustos carne/jamón y queso, en prod) + **12.3** (tickets alumno-primero + adulto al lado + desglose de gustos, local-only). App lista para difundir.
- **Deploy**: branch `feature/fase-12-combo-empanadas` mergeado a main (commit `d53aa2c`) → `./scripts/deploy.sh`. Cloud Run **revision `00022-hzf`** (12.1+12.2). **Smoke prod verde**: config (rifa id=3, $1000, activa) + 2000/2000 disponibles + home 200 + E2E pago: order empanadas → preference MP con `back_urls`/`notification_url` en el dominio Cloud Run (BUG-010 OK) + item "Combo de empanadas — 1 carne 1 j&q" $12.000. Order de prueba del smoke limpiado.
- **Cambio de fecha de cierre**: a pedido del usuario, de 30/6 → **lunes 6/7 23:59 ART** (`SALES_CLOSE_TS` en `RifasApp.tsx`). Re-deploy **revision `00023-zxz`**, verificado el string `2026-07-06T23:59:00-03:00` en el bundle servido.
- **Schema**: ALTER additive `flavor_breakdown TEXT` (nullable JSON) en `combo_purchase_items`, aplicado a Turso prod vía MCP (BD sede 2 vacía, 0 riesgo). `quantity`=nº combos y `unitPrice`=$12.000 SIN cambios → flujo de pago intacto. Migración formal `drizzle/0001_ambitious_shadow_king.sql`.
- **Reviews**: `db-migration-reviewer` APROBADO CON OBSERVACIONES (atendidas) · `payment-flow-debugger` APROBADO (anti-tampering, idempotencia, BUG-012/013 OK, 1 minor de hardening aplicado).
- **Validación**: E2E local + smoke prod (12.2); render mock con Chrome headless de 4 casos de ticket (12.3: rifa+combo, combo-only con gustos, solo-combos sin alumno, solo-rifa) — alumno destacado + "compró: <adulto>" + chips "Carne ×N / J. y queso ×N". Lint + build verdes en cada paso.
- **BD al cierre**: rifa id=3 abierta, 2000 números disponibles, 0 ventas approved. Queda 1 order de prueba del usuario (`ORD-RZh5Xh9_cr`, "Rodrigo Test", rifa #1 + 2 combos $25.000) auto-cancelado por el cron a los 15min (número #1 liberado) — **el usuario decidió dejarlo** (no aparece en grilla ni reportes que filtran por approved). Confirmó que el cron de timeout + persistencia de gustos funcionan en prod con un order real.
- **Próxima**: 12.4 (talonario de no vendidas — espera medidas del talón + lema del colegio) + compra real E2E con un tercero (no Rodrigo por seller=buyer).
- **Bugs nuevos**: ninguno.
- **Notas**: la sesión tocó el flujo de pago (combo) → ya validado con smoke E2E prod (inspección URLs preference MP) + `payment-flow-debugger`; resta solo la compra real con dinero de un tercero.
