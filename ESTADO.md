...
- **Última sesión**: 2026-06-12 — Fase 12.1 + 12.2 implementadas, validadas y deployadas (combo empanadas con gustos + cierre 6/7). Schema `flavor_breakdown` aplicado a prod, reviews db-migration + payment-flow APROBADAS, smoke prod E2E verde (BUG-010 OK). Falta solo compra real de tercero. Backup de las 8 tablas (sucesor del script viejo que sólo cubría 5) + reset destructivo atómico revisado por `db-migration-reviewer` (APROBADO) + rifa nueva id=3 + redeploy `00021-fdd` + smoke prod verde (2000/2000 disponibles). Removido el gate `SALES_CLOSE_TS` de sede 1. Plan completo de los 5 pedidos de los directores de sede 2 en `docs/superpowers/plans/2026-06-09-fase-12-cambios-sede2.md` (el #5 se fusionó en el combo). Decisiones del combo cerradas: único combo empanadas, suma de gustos exacta N×2.
...
### 2026-06-12 — Fase 12.1 + 12.2 DEPLOYADAS (combo empanadas con gustos + cierre 6/7)
- **Tareas completadas y EN PRODUCCIÓN (revision `00022-hzf` → re-deploy con fecha 6/7)**: 12.1 (cierre auto 6/7 23:59) + 12.2 (combo de empanadas $12.000 con reparto exacto de gustos carne/jamón y queso). App lista para difundir.
- **Branch**: `feature/fase-12-combo-empanadas` mergeado a main (fast-forward, commit `d53aa2c`) + push.
- **Deploy**: `./scripts/deploy.sh` → Cloud Run revision `00022-hzf`. **Smoke prod verde**: config (rifa id=3, $1000, activa) + 2000/2000 disponibles + home 200 + E2E pago: order empanadas creado → preference MP con `back_urls`/`notification_url` apuntando al dominio Cloud Run (BUG-010 OK) + item "Combo de empanadas — 1 carne 1 j&q" $12.000 qty 1. Order de prueba limpiado, BD sede 2 en 0 ventas.
- **Schema**: ALTER additive `flavor_breakdown TEXT` (nullable JSON) en `combo_purchase_items`, aplicado a Turso prod vía MCP (BD sede 2 estaba vacía, 0 riesgo). `quantity`=nº combos y `unitPrice`=$12.000 SIN cambios → flujo de pago intacto. Migración formal generada (`drizzle/0001_ambitious_shadow_king.sql`).
- **Reviews**: `db-migration-reviewer` APROBADO CON OBSERVACIONES (todas atendidas) · `payment-flow-debugger` APROBADO (anti-tampering OK, idempotencia OK, BUG-012/013 respetados, 1 minor de hardening aplicado).
- **Validación E2E local** (dev contra BD prod, registros de prueba luego borrados): order válido → total $24.000, `flavor_breakdown` persistido `{"carne":3,"jyq":1}`, `created_at` integer; preference MP creado OK con desglose en el título; inputs inválidos (suma≠N×2, sin flavors) → HTTP 400 + rollback total. Lint + build verdes.
- **Próxima**: compra real de validación con un tercero (no Rodrigo por seller=buyer) para cerrar el ciclo de pago end-to-end con dinero real. Luego 12.3 (tickets alumno-primero) y 12.4 (talonario no vendidas, espera medidas + lema).
- **Bugs nuevos**: ninguno.
